(ns rust-sentence-transformers-clj.core
  (:import ClojureRust))



(clojure.lang.RT/loadLibrary "rust_sentence_transformers_clj")

(defn get-sentence-transformer
  [model-path device]
  (ClojureRust/getSentenceTransformer model-path device))
...
(defn encode
  [embedder documents batch-size]
  (ClojureRust/encode embedder documents batch-size))